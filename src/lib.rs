...
impl Canvas {
    ...
    fn rasterize_filled_rectangle(&mut self, p1: Point2, p2: Point2, p3: Point2, p4: Point2) {
        // Let's calculate the bounding box, rounding to the nearest integer in the process
        let points = [p1, p2, p3, p4];
        let xs = points.iter().map(|p| p.get_x()).collect::<Vec<_>>();
        let ys = points.iter().map(|p| p.get_y()).collect::<Vec<_>>();

        let min_x = helper_get_min(xs.clone()).unwrap().floor() as i32 - 1;
        let max_x = helper_get_max(xs).unwrap().ceil() as i32 + 1;
        let min_y = helper_get_min(ys.clone()).unwrap().floor() as i32 - 1;
        ...
        for y in min_y..=max_y {
            for x in min_x..=max_x {
                ...
                if self.antialias_enabled {
                    let x_fac = x as f32;
                    let y_fac = y as f32;
                    let corner_offsets = [ [0f32, 0f32],
                                           [3f32/4f32, 0f32],
                                           [3f32/4f32, 3f32/4f32],
                                           [0f32, 3f32/4f32] ];
                    let corner_locs = corner_offsets.iter()
                        .map(|[xoff, yoff]| [x_fac + xoff, y_fac + yoff])
                        .collect::<Vec<_>>();
                    let corners_inside = corner_locs.iter()
                    ...
                    if is_full {
                        ...
                        self.backing.set_pixel(x as u64, y as u64, &self.current_color).unwrap();
                        continue;
                    }
                    ...
                    let mut subs_within_polygon = vec![ false; subpixels_per_side * subpixels_per_side ];
                    ...
                    let aa_blend_proportion = subs_within_polygon.into_iter()
                    ...
                    let out_r = (blent_color.r.powf(gamma) * blent_color.a +
                                 cur_color.r.powf(gamma) * (1f32 - blent_color.a)).powf(1f32/gamma);
                    let out_g = (blent_color.g.powf(gamma) * blent_color.a +
                                 cur_color.g.powf(gamma) * (1f32 - blent_color.a)).powf(1f32/gamma);
                    let out_b = (blent_color.b.powf(gamma) * blent_color.a +
                                 cur_color.b.powf(gamma) * (1f32 - blent_color.a)).powf(1f32/gamma);
                    ...
                    self.backing.set_pixel(x as u64, y as u64, &out_color).unwrap();
                } else {
